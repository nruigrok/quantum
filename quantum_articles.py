...
df = pd.read_excel(r'~/Dropbox/quantum/data/PSSet.xlsx', sheet_name = 'Sheet1')
topics = pd.read_excel(r'~/Dropbox/quantum/data/topics.xlsx', sheet_name = 'Sheet1')
topics['Article ID'] = topics['Article #']

df3=df.merge(topics, on="Article ID", how='left')
print(df3)
df3["label"] = df3["topic"]
df3["text"] = df3["Body"]

d = df3[["text", "label"]]
d.to_csv('data/quantum_topics.csv')
